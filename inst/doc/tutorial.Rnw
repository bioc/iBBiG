...
Statistical measures of biclustering performance including Chia and Karuturi Function, Coherence measures and F Statistics.

<<biclusStat, eval=FALSE, echo=FALSE>>=
## Need to fix bug that @RowxNumber and @NumberxCol are binary instead of logical in iBBiG... simple enough and then more biclust functions  should work
ChiaKaruturi(res@Seeddata, res, 4)
@
